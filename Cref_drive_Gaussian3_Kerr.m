...
for n = 1:timeN
    ...
    for n20 = 1:7
        ...
        for n1 = 1:ucNum
            ...
            c1temp1 = c1temp+d1temp*((yi(n20,4*n1-1))^2+(yi(n20,4*n1-0))^2);
            c1temp2 = c1temp+d1temp*((yi(n20,4*n1-3))^2+(yi(n20,4*n1-2))^2);
            
            
            Fa  = e0*yi(n20,4*n1-2)+ d*yi(n20,4*n1-2)^3;
            F1a = c1temp1*yi(n20,4*n1+0);
            if n1 == 1
                F2a = 0;
            else
                c2temp1 = c2temp+d2temp*((yi(n20,4*n1-5))^2+(yi(n20,4*n1-4))^2);
                F2a = c2temp1*yi(n20,4*n1-4);
            end
            Ga  = +Fa+F1a+F2a-f(4*n1-3)*sin(w*n*dt+phase(4*n1-3))*gFactor-eta*yi(n20,4*n1-3);
            
            Fb  = e0*yi(n20,4*n1-3)+ d*yi(n20,4*n1-3)^3;
            F1b = c1temp1*yi(n20,4*n1-1);
            if n1 == 1
                F2b = 0;
            else
                c2temp1 = c2temp+d2temp*((yi(n20,4*n1-5))^2+(yi(n20,4*n1-4))^2);
                F2b = c2temp1*yi(n20,4*n1-5);
            end
            Gb  = -Fb-F1b-F2b-f(4*n1-2)*sin(w*n*dt+phase(4*n1-2))*gFactor-eta*yi(n20,4*n1-2);
            
            Fc  = e0*yi(n20,4*n1+0)+ d*yi(n20,4*n1+0)^3;
            F1c = c1temp2*yi(n20,4*n1-2);
            if n1 == ucNum
                F2c = 0;
            else
                c2temp2 = c2temp+d2temp*((yi(n20,4*n1+1))^2+(yi(n20,4*n1+2))^2);
                F2c = c2temp2*yi(n20,4*n1+2);
            end
            Gc  = +Fc+F1c+F2c-f(4*n1-1)*sin(w*n*dt+phase(4*n1-1))*gFactor-eta*yi(n20,4*n1-1);
            
            Fd  = e0*yi(n20,4*n1-1)+ d*yi(n20,4*n1-1)^3;
            F1d = c1temp2*yi(n20,4*n1-3);
            if n1 == ucNum
                F2d = 0;
            else
                c2temp2 = c2temp+d2temp*((yi(n20,4*n1+1))^2+(yi(n20,4*n1+2))^2);
                F2d = c2temp2*yi(n20,4*n1+1);
            end
...
figure(7); clf;
hold on;
H1 = plot([10 20],[10 20],'color',red,'linewidth',LineWidth);
H2 = plot([10 20],[10 20],'color',blue,'linewidth',LineWidth);
H3 = plot([10 20],[10 20],'color',yellow,'linewidth',LineWidth);
hold off;
legend([H1 H2 H3],{'${\rm Re\,}\Psi_1^{(1)}$','${\rm Re\,}\Psi_2^{(1)}$','${\rm Re\,}\Psi_3^{(1)}$'},'Location','northeast','FontSize',21,'Interpreter','latex');
set(gca,'linewidth',2)
set(gca,'FontSize',19)
xlabel('$t/T$','Interpreter','latex')
ylabel('$\Psi_n^{(1)}(t)$','Interpreter','latex')
...
figure(8); clf;
hold on;
H1 = plot((1:timeN)*dt/(2*pi/e0),fRec(:,1),'b-','linewidth',LineWidth,'color',brown);
...
H1 = plot(FreqMeasure2(1:fftLabelLimit),GaussFFT(1:fftLabelLimit)/max(GaussFFT(1:fftLabelLimit)),'b-','linewidth',LineWidth,'color',brown);
...
% H1 = plot(FreqMeasure2(1:fftLabelLimit),0.05*GaussFFT(1:fftLabelLimit)/max(GaussFFT(1:fftLabelLimit)),'b--','linewidth',LineWidth,'color',brown);
H2 = plot(FreqMeasure(1:fftLabelLimit),ampwz(1:fftLabelLimit,81),'b-','linewidth',3);
% plot([+c1-c2 +c1-c2],[0 3e-4],'k--','linewidth',LineWidth)
% plot([-c1+c2 -c1+c2],[0 3e-4],'k--','linewidth',LineWidth)
hold off;
legend([H2],{'$\Psi_{20}^{(1)}(\omega)$'},'Location','northeast','FontSize',16,'Interpreter','latex');
%legend([H1 H2],{'${\rm Re\,}S_1(\omega)$','${\rm Re\,}\Psi_1^{(1)}(\omega)$'},'Location','northeast','FontSize',16,'Interpreter','latex');
set(gca,'linewidth',2)
set(gca,'FontSize',19)
xlabel('$\omega$','Interpreter','latex')
ylabel('FFT','Interpreter','latex')
xticks([2*e0/3 e0 4*e0/3])
xticklabels({'1','\epsilon_0=1.5','2'})
yticks([0 0.01])
axis([2*e0/3 4*e0/3 0 0.01])
% set(gca, 'YScale', 'log')
...
for n = 1:ucNum-1
    ...
    BCube = 0;
    CCube = c2temp/d2temp;
    DCube = -d1temp/d2temp*Apre^3-c1temp/d2temp*Apre;
...
colors(2,1:3) = [28 134 238]/256;
colors(3,1:3) = [0 0 255]/256;
colors(4,1:3) = [238 0 0]/256;
colors(5,1:3) = [238 106 167]/256;
colors(6,1:3) = [255 181 197]/256;
LineWidth = 1.5;
...
nA = 0;
for A = [0 nan 0.5*Ac Ac 1.2*Ac nan]
    nA = nA+1;
    c1temp = c1+d1*A^2;
    c2temp = c2+d2*A^2;
    for nq = 1:Nq+1
        q = 2*pi/Nq*(nq-1);
        w1(nq,nA) = e0+sqrt(c1temp^2+c2temp^2+2*c1temp*c2temp*cos(q));
        w2(nq,nA) = e0-sqrt(c1temp^2+c2temp^2+2*c1temp*c2temp*cos(q));
    end
end

nA = 1;
H1 = plot((0:Nq)*2*pi/Nq,w1(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth);
plot((0:Nq)*2*pi/Nq,w2(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth)

% nA = 2;
% H2 = plot((0:Nq)*2*pi/Nq,w1(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth);
% plot((0:Nq)*2*pi/Nq,w2(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth)

nA = 3;
H3 = plot((0:Nq)*2*pi/Nq,w1(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth);
plot((0:Nq)*2*pi/Nq,w2(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth)

nA = 4;
H4 = plot((0:Nq)*2*pi/Nq,w1(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth);
plot((0:Nq)*2*pi/Nq,w2(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth)

nA = 5;
H5 = plot((0:Nq)*2*pi/Nq,w1(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth);
plot((0:Nq)*2*pi/Nq,w2(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth)

% nA = 6;
% H6 = plot((0:Nq)*2*pi/Nq,w1(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth);
% plot((0:Nq)*2*pi/Nq,w2(1:Nq+1,nA),'color',colors(nA,:),'linewidth',LineWidth)

plot([0 2*pi],[1.5 1.5],'k--','linewidth',LineWidth)

hold off;
legend([H1 H3 H4 H5],{'$A=0$','$A=0.5A_c$','$A=A_c$','$A=1.2A_c$'},'Location','northeast','FontSize',16,'Interpreter','latex');
set(gca,'linewidth',2)
set(gca,'FontSize',19)
