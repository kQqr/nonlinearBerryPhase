...
if figSee == 1
    ...
    H2 = plot(dTk*(1:timeN),cosCompare(1:timeN),'k-.','linewidth',3);
    H3 = plot(dTk*(1:timeN),zk(1:timeN,4*(n1+1-1)+1),'b-','linewidth',3);
    axis([0 Tk -maxzk1 maxzk1])
    hold off;
    legend([H1 H2 H3],{'${\rm Re\,}\Psi_1^{(1)}(t)$','$A\cos(\omega t)$','${\rm Re\,}\Psi_2^{(1)}(t)$'},'Location','east','FontSize',16,'Interpreter','latex');
    set(gca,'linewidth',2)
...
%     alpha(alphaValue)
    ...
    legend(H2,{'${\rm Re\,}\Psi_1^{(1)}(\omega)$'},'Location','east','FontSize',16,'Interpreter','latex');
    xticks([0 5 10])
...
H1 = plot(dTk*(1:size(zfft1,1))/Tk,zfft1(:),'b-','linewidth',3);

axis([0 dTk*size(zfft1,1)/Tk+0.1 -1.6 1.6])
hold off;
set(gca,'linewidth',2)
set(gca,'FontSize',19)
xticks([0 10 20])
...
legend(H1,{'${\rm Re\,}\Psi_1^{(1)}(t)$'},'Location','east','FontSize',16,'Interpreter','latex');
