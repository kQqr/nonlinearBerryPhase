...
if abs((maxzkEnd-maxzk(1))/maxzk(1)) < tolmaxzk
    ...
    % zfftTot(timeN*(fftNum-1)+2,1)-zfftTot(2,1)
    ...
    if figSee == 1
        ...
        for fftLabel = 1:4
            ...
            FreqMeasureNorm = FreqMeasure(1:end)/NNM(4*(NNMlabel-1)+2,2);
            ...
            H2 = plot(FreqMeasureNorm(2:fftLabelLimit),P1x(2:fftLabelLimit),'b-','linewidth',3);
            ...
            xticks([0 10 20])
            yticks([1e-6 1e-3 1])
            axis([0 fftOrder min(P1x) max(P1x)])
        end
    ...
    H1 = plot(dTk*(1:size(zfftTot,1))/Tk,zfftTot(:,1),'b-','linewidth',0.5);
    
    axis([0 dTk*size(zfftTot,1)/Tk -1.6 1.6])
    hold off;
    set(gca,'linewidth',2)
    set(gca,'FontSize',19)
    xticks([0 100 200])
    ...
    labelPsiRec = labelPsiRec+1;
    for fftLabel = 1:4
        psiLRec(2*labelPsiRec-1,(fftLabel-1)*fftOrder+1:fftLabel*fftOrder) = transpose(psiL(1:fftOrder,fftLabel));
        psiLRec(2*labelPsiRec-0,1) = NNM(4*(NNMlabel-1)+2,1); % maxzk(1);
        psiLRec(2*labelPsiRec-0,2) = NNM(4*(NNMlabel-1)+2,2); % 2*pi/Tk;
        psiLRec(2*labelPsiRec-0,3) = mean(NNM(4*(loopA-1)+3,1:DOF)); % q;
        psiLRec(2*labelPsiRec-0,4) = (sum(NNM(4*(NNMlabel-1)+4,3:4:end))-sum(NNM(4*(NNMlabel-1)+4,1:4:end)))/(size(NNM,2)/4); % transpose(IntraPhase(1:DOF))
        
        %     NNM(4*(loopA-1)+1,1:DOF) = zk(1,1:DOF);
        %     NNM(4*(loopA-1)+2,1) = maxzk(1);
        %     NNM(4*(loopA-1)+2,2) = 2*pi/Tk;
        %     NNM(4*(loopA-1)+2,3) = errorFunc;
        %     NNM(4*(loopA-1)+3,1:DOF) = mean(transpose(InterPhase(1:DOF)));
